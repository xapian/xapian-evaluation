...
void flush_outbuf()
{
  /* In PADRE, this is a NOOP, because the output buffer is never emptied */
  return;

    if (outcnt == 0) return;

    write_buf(ofd, (char *)outbuf, outbufptr);
    bytes_out += (ulg)outbufptr;
    outbufptr = 0;
}
    ...
    unsigned  cnt;
{
    ...
    if ((outbufptr + cnt) > outbuflen) {
      fprintf(stderr, "Error:  Decompressed file too big %d + %d\n",
	      outcnt, cnt);
      exit_code = ERROR;
      return;
    } else {
      memcpy(outbuf+outbufptr, buf, cnt);
      outbufptr += cnt;
    }
}
...
void error(m)
    char *m;
{
  fprintf(stderr, "Error: Damnation!\n");
  return;
    fprintf(stderr, "\n%s: %s: %s\n", progname, ifname, m);
    /* abort_gzip();  */
}
